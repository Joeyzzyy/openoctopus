...
const apiName = "POST /v1/chat/completions";
...
async function readCases() {
  const suite = readArg("suite", "deepseek-v4-pro");
  const casePath = path.resolve(__dirname, `../${suite}/cases/chat.completions.json`);
  const raw = await readFile(casePath, "utf8");
  return { suite, cases: JSON.parse(raw) };
}

function readOutputText(taskResponse) {
  const payload =
    taskResponse && typeof taskResponse === "object" && !Array.isArray(taskResponse)
      ? taskResponse.output_payload
      : null;
...
async function runCase({ smokeCase, apiKey, keyPrefix, baseUrl, suite }) {
  ...
  const baseRecord = {
    api: apiName,
    script: "tests/api-smoke/scripts/chat.completions.mjs",
    suite,
    caseId: smokeCase.id,
  ...
  try {
    ...
    const outputText = readOutputText(task);
    ...
    if (outputText.trim().length < minTextLength) {
      throw new Error(`Expected at least ${minTextLength} characters of output text, received ${outputText.trim().length}`);
    }
