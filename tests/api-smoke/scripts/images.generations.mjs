...
const apiName = "POST /v1/images/generations";
...
async function readCases() {
  const suite = readArg("suite", "imagen-4");
  const casePath = path.resolve(__dirname, `../${suite}/cases/images.generations.json`);
...
async function runCase({ smokeCase, apiKey, keyPrefix, baseUrl, suite }) {
  ...
  const baseRecord = {
    api: apiName,
    script: "tests/api-smoke/scripts/images.generations.mjs",
    suite,
    caseId: smokeCase.id,
  ...
  try {
    ...
    if (submitStatus !== smokeCase.expect?.submitStatus) {
      const message = json?.error?.message || json?.message || `Expected submit HTTP ${smokeCase.expect?.submitStatus}, received ${submitStatus}`;
