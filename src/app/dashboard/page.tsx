...
import { getDashboardData, type DashboardData } from "@/lib/dashboard-server";
...
type DashboardRequestRow = DashboardData["requestQueueRows"][number];
const pageNav = [
  { view: "dashboard" },
  { view: "explore" },
  { view: "api-keys" },
...
function parseRequestGroupId(value: string | undefined): string | null {
...
function buildDashboardHref(input: {
  ...
  params.set("requestDetailsPage", String(input.requestDetailsPage ?? 1));
  ...
  if (input.requestGroupId) {
    params.set("requestGroup", input.requestGroupId);
  }
...
function RequestRecordsTable({
  ...
  expandedRows = [],
  expandedPagination,
  expandedPaginationNode,
  emptyDetail,
}: {
  rows: DashboardRequestRow[];
  labels: ReturnType<typeof getI18n>["dashboard"]["requests"];
  operationForRow: (row: DashboardRequestRow) => React.ReactNode;
  expandedRowId?: string | null;
  expandedRows?: DashboardRequestRow[];
  expandedPagination?: {
    page: number;
    pageSize: number;
    total: number;
  ...
  expandedPaginationNode?: React.ReactNode;
  emptyDetail?: string;
}) {
  const renderDetailOperation = (row: DashboardRequestRow) =>
    row.promptText ? (
      <RequestPromptDialogButton
        prompt={row.promptText}
        buttonLabel={labels.viewPrompt}
        title={labels.promptDialogTitle}
        description={labels.promptDialogDescription}
        closeLabel={labels.closePromptDialog}
      />
    ) : null;
  const renderExpandedDetails = () => (
    <div className="rounded-lg border border-[#BAE6FD] bg-white">
      <div className="flex items-center justify-between gap-3 border-b border-[#BAE6FD] px-3 py-2">
        <div>
          <p className="text-xs font-medium text-black">{labels.sessionDetails}</p>
          {expandedPagination ? (
            <p className="mt-0.5 text-[11px] text-black/50">
              {formatI18n(labels.perPage, { count: expandedPagination.pageSize })}
            </p>
          ) : null}
        </div>
        {expandedPagination ? (
          <span className="text-[11px] text-black/45">
            {formatI18n(labels.page, {
              page: expandedPagination.page,
              total: expandedPagination.totalPages,
            })}
          </span>
        ) : null}
      </div>
      <RequestRecordsTable
        rows={expandedRows}
        labels={labels}
        operationForRow={renderDetailOperation}
  ...
  return (
    <>
      <div className="space-y-2 md:hidden">
        {rows.length > 0 ? (
          rows.map((row) => {
            const isExpanded = expandedRowId === row.requestId;
            return (
              <article
                key={row.requestId}
                className={cn(
                  "rounded-2xl border p-3 shadow-sm",
                  row.rowKind === "session" ? "border-[#38BDF8] bg-[#F0F9FF]" : "border-black/[0.08] bg-[#F8FCFF]"
                )}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-black/45">
                      {row.rowKind === "session"
                        ? formatI18n(labels.sessionRequests, { count: row.requestCount })
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-black/70">
                  <div>{labels.time}: {row.createdAtLabel}</div>
                  <div>{labels.vendor}: {row.vendor}</div>
                  <div>{labels.latency}: {row.latency}</div>
                  <div>{labels.cost}: {formatRequestCostLabel(row)}</div>
                </div>
                {row.promptText ? (
                  <div className="mt-3 rounded-xl border border-black/[0.08] bg-white px-3 py-2">
                    <p className="text-[11px] tracking-[0.35px] text-black/45">{labels.prompt}</p>
                    <p
                ...
                <div className="mt-3 flex justify-end">{operationForRow(row)}</div>
                {isExpanded ? <div className="mt-3">{renderExpandedDetails()}</div> : null}
              </article>
            );
          })
        ) : (
          <EmptyState title={labels.emptyTitle} detail={emptyDetail ?? labels.emptyDetail} />
        )}
      </div>
                ...
                {[
                  ...
                  <th
                    ...
                    className={cn(
                      ...
                      heading === labels.operation ? "text-right" : ""
                    )}
                  >
                    {heading}
                  </th>
                ...
                rows.map((row) => {
                  ...
                  return (
                    <Fragment key={row.requestId}>
                      <tr
                        className={cn(
                          "border-b border-black/10 transition-colors hover:bg-black/[0.02]",
                          row.rowKind === "session" && "bg-[#F0F9FF]"
                        )}
                      >
                        ...
                        <td className="px-2 py-3 text-xs text-black/60">
                          <span className="block max-w-[120px] truncate" title={row.createdAtTitle}>
                            {row.createdAtLabel}
                          </span>
                        </td>
                      ...
                      {isExpanded ? (
                        <tr key={`${row.requestId}-details`} className="border-b border-[#BAE6FD] bg-[#F0F9FF]">
...
export default async function DashboardPage({
  ...
  const rawRequestDetailsPage = Number(getSearchValue(resolvedSearchParams, "requestDetailsPage") ?? "1");
  const requestDetailsPage = Number.isFinite(rawRequestDetailsPage)
    ? Math.max(1, Math.floor(rawRequestDetailsPage))
    : 1;
  const selectedRequestGroupParam = parseRequestGroupId(getSearchValue(resolvedSearchParams, "requestGroup"));
  ...
  const data = await getDashboardData({
    requestsPage,
    requestDetailsPage,
    requestGroupId: selectedRequestGroupParam,
    includeRequestDetails: view === "request-details",
    billingPage,
  ...
  const {
    ...
    data ?? {
      ...
      requestDetailsPagination: { page: 1, pageSize: 10, total: 0, totalPages: 1 },
            ...
            {view === "request-details" ? (
              <section className="p-0">
                <RequestRecordsTable
                  ...
                  labels={dashboardCopy.requests}
                  operationForRow={(row) => {
                    if (row.rowKind === "session") {
                      const isExpanded = selectedRequestGroupId === row.requestId;
                      return (
                        <Link
                          href={buildDashboardHref({
                            view: "request-details",
                            requestsPage: requestPagination.page,
                            requestDetailsPage: 1,
                            requestGroupId: isExpanded ? null : row.requestId,
                            apiKeyId: selectedApiKeyId,
                            analyticsInterval,
                            analyticsRange,
                          })}
                          className="inline-flex h-8 items-center gap-1 rounded-md border border-[#BAE6FD] bg-white px-2 text-xs font-medium text-[#075985] transition-colors hover:bg-[#E0F2FE]"
                        >
                          {isExpanded ? <ChevronDown className="size-3.5" /> : <ChevronRight className="size-3.5" />}
                          {isExpanded ? dashboardCopy.requests.collapse : dashboardCopy.requests.expand}
                        </Link>
                      );
                    }
                  ...
                  expandedRowId={selectedRequestGroupId}
                  expandedRows={requestDetailRows}
                  expandedPagination={requestDetailsPagination}
                  expandedPaginationNode={
                    requestDetailsPagination.totalPages > 1 ? (
                      <div className="flex flex-col gap-3 border-t border-[#BAE6FD] px-3 py-3 md:flex-row md:items-center md:justify-between">
                        <div className="flex items-center gap-2">
                          <Link
                            href={buildDashboardHref({
                              view: "request-details",
                              requestsPage: requestPagination.page,
                              requestDetailsPage: Math.max(1, requestDetailsPagination.page - 1),
                              requestGroupId: selectedRequestGroupId,
                              apiKeyId: selectedApiKeyId,
                              analyticsInterval,
                              analyticsRange,
                            })}
                            aria-disabled={requestDetailsPagination.page <= 1}
                            className={cn(
                              ...
                              requestDetailsPagination.page <= 1 && "pointer-events-none opacity-40"
                            )}
                          >
                            {dashboardCopy.requests.previous}
                          </Link>
                          <div className="flex items-center gap-2">
                            {buildVisibleRequestPages(requestDetailsPagination.page, requestDetailsPagination.totalPages).map(
                              (page, index, pages) => (
                                <div key={page} className="flex items-center gap-2">
                                  ...
                                  <Link
                                    ...
                                    className={cn(
                                      ...
                                      page === requestDetailsPagination.page
                                        ? activePillClassName
                          ...
                          <Link
                            href={buildDashboardHref({
                              view: "request-details",
                              requestsPage: requestPagination.page,
                              requestDetailsPage: Math.min(
                                requestDetailsPagination.totalPages,
                                requestDetailsPagination.page + 1
                              ),
                              requestGroupId: selectedRequestGroupId,
                              apiKeyId: selectedApiKeyId,
                              analyticsInterval,
                            ...
                            aria-disabled={requestDetailsPagination.page >= requestDetailsPagination.totalPages}
                            className={cn(
                              "inline-flex h-8 items-center rounded-md border border-[#BAE6FD] bg-white px-3 text-xs font-medium text-[#075985] transition-colors hover:bg-[#E0F2FE]",
                              requestDetailsPagination.page >= requestDetailsPagination.totalPages &&
                        ...
                        <span className="text-xs text-black/50">
                          {formatI18n(dashboardCopy.requests.page, {
                            page: requestDetailsPagination.page,
                            total: requestDetailsPagination.totalPages,
                          })}
                        </span>
                      </div>
                    ) : null
                          ...
                          (page, index, pages) => (
                            <div key={page} className="flex items-center gap-2">
                              ...
                              <Link
                                ...
                                className={cn(
                                  "inline-flex h-8 min-w-8 items-center justify-center rounded-md border px-2 text-xs font-medium transition-colors",
                                  page === requestPagination.page ? activePillClassName : inactivePillClassName
                                )}
                              >
                                {page}
                              </Link>
